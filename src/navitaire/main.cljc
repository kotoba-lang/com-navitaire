(ns navitaire.main
  "Kotodama WASM entrypoint for the Navitaire clean-room actor (L4) — Clojure port.
  ...
  facts (`navitaire.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "navitaire")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"navitaire.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "navitaire-compat" :tier tier :entities entities} 200])
...
(defn start! [] :navitaire-compat/ready)
